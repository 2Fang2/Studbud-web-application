...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"js/readinglist.js":[function(require,module,exports) {
// page dom
var date = document.getElementById("date"),
    ...
    copyreadingList = [];
date.value = getToday(); // set date from tody

readingListQuery(date.value); // Get current time
...
} // Determine whether it is the same day
...
function readingListQuery(date, openStatus, copyreadingList) {
  var readingListArr = JSON.parse(localStorage.getItem("readingList"));
  ...
  if (readingListArr != null) {
    ...
    readingListArr.forEach(function (item, index) {
      var id = item.id,
          links = item.links,
          time = item.time,
          ckeck = item.ckeck;
...
addLinkButton.addEventListener("click", function () {
  var linksegValue = linkseg.value,
  ...
  if (readingList == null) {
    ...
    var _readingListArr = JSON.parse(readingList);

    obj = {
      "id": _readingListArr.length,
      "links": linksegValue,
      "ckeck": false,
      "time": getToday()
    };

    _readingListArr.push(obj);

    localStorage.setItem("readingList", JSON.stringify(_readingListArr));
  }
...
saveButton.addEventListener("click", function () {
  if (copyreadingList && copyreadingList.length > 0) {
    var checkboxInput = document.querySelectorAll("input[name='editInput']");
    checkboxInput.forEach(function (item, index) {
      var i = copyreadingList.findIndex(function (v) {
        return v.id == item.getAttribute("data-id");
      });
...
deletedButton.addEventListener("click", function () {
  ...
  checkboxInput.forEach(function (item, index) {
    if (item.checked) {
      var _index = readingListArr.findIndex(function (v) {
        return v.id == item.getAttribute("data-id");
      });

      readingListArr.splice(_index, 1);
      localStorage.setItem("readingList", JSON.stringify(readingListArr));
    }
  });
  readingListQuery(date.value);
...
},{}]},{},["../../../AppData/Roaming/npm/node_modules/parcel-bundler/src/builtins/hmr-runtime.js","js/readinglist.js"], null)
//# sourceMappingURL=/readinglist.2e49f899.js.map